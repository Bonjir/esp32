...
#                                                         0x30, 0x39, 0x3f, 0x00, 0x07, 0x03, 0x10])
    ...
    def char( self, aPos, aChar, aColor, aSize = (1, 1), bRewrite = False):
            ...
            for col in range(0, self.fontw):
                for pixel in range(0, self.fonth):
                    ...
                    if b:
                        ...
                        if aSize == 1:
                            self.pixel((aPos[0] + col, aPos[1] + pixel), self.aBkgndClr)
                        else:
                            self.fillrect((aPos[0] + col * aSize[0], aPos[1] + pixel * aSize[1]), (aSize[0], aSize[1]), self.aBkgndClr )
        else :
            self.fillrect((aPos[0] + 1 * aSize[0], aPos[1]), ((self.fontw - 1) * aSize[0], (self.fonth - 1) * aSize[1]), aColor)
            
    def textout( self, aPos, aStr, aColor, auto_return = True, aSize = (1, 1), bRewrite = False):
        aSize_scale = aSize
        aSize = (aSize[0] * self.font_size[0], aSize[1] * self.font_size[1])
        aPos0 = aPos
        screen_width = 0
        ...
        for i in range(0, len(aStr)):
            if aStr[i] == '\n':
                ...
                continue
            if auto_return and aPos[0] + (i + 1) * self.fontw * aSize[0] >= screen_width:
                aPos = (aPos0[0] - i * self.fontw * aSize[0], aPos[1] + self.fonth * aSize[1])
            self.char((aPos[0] + i * self.fontw * aSize[0], aPos[1]), aStr[i], aColor, aSize_scale, bRewrite)
